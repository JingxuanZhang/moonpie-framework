...
class ArrayDataProvider extends BaseDataProvider
{
    ...
    protected function prepareModels()
    {
        ...
        if($pagination === false) {
            $this->setPagination($this->allModels, $this->paginationConfig);
            $pagination = $this->getPagination();
        }
        if ($pagination !== false) {
            $page_size = $pagination->listRows();
            if ($page_size > 0) {
                $current_page = $pagination->currentPage();
                $models = array_slice($models, max(0, $current_page - 1) * $page_size, $page_size, true);
            }
        }
